...
export default function HitungGajiForm() {
  const [gaji, setGaji] = useState(""); // Awalnya kosong
  const pajak = 0.11;

  // Konversi input ke angka, atau kosong jika input dihapus
  const handleInputChange = (e) => {
    const value = e.target.value;
    setGaji(value === "" ? "" : Number(value));
  };

  const totalGaji = gaji > 0 ? gaji - gaji * pajak : "";
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg w-96">
        ...
        {gaji === "" || gaji <= 0 ? (
          ...
          <div className="mt-4 p-3 bg-blue-100 border-l-4 border-blue-500 text-blue-700">
            <p className="font-semibold">
              Total Take Home Pay (THP): Rp {parseFloat(totalGaji).toLocaleString()}
            </p>
          </div>
        )}
      </div>
